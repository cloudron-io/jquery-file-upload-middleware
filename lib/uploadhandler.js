...
module.exports = function (options) {
            ...
            .on('file', function (name, file) {
                ...
                fs.exists(file.path, function(exists) {
                    if (exists) {
                        ...
                        var generatePreviews = function () {
                            if (options.imageTypes.test(fileInfo.name)) {
                                _.each(options.imageVersions, function (value, version) {
                                    ...
                                    mkdirp(options.uploadDir() + '/' + version + '/').then(function (made) {
                        ...
                        mkdirp(options.uploadDir() + '/').then(function (made) {
    ...
    UploadHandler.prototype.destroy = function () {
        ...
        fs.unlink(filepath, function (ex) {
            _.each(options.imageVersions, function (value, version) {
                fs.unlink(path.join(options.uploadDir(), version, fileName), NOOP_FUNC);
            });
            self.emit('delete', fileName);
